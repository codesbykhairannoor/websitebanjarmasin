...
langs.forEach(lang => {
  ...
  let langStartIndex = lines.findIndex(l => l.startsWith(`  ${lang}: {`));
  if (langStartIndex === -1) return;
  // If en, we skip the duplicate! Wait, we can just find the LAST index of '  en: {' which is the real one?
  if (lang === 'en') {
    langStartIndex = lines.findIndex((l, i) => i > langStartIndex && l.startsWith('  en: {'));
  }
  
  // From langStartIndex, find the first 'panduan: {'
  let panduanIndex = lines.findIndex((l, i) => i > langStartIndex && l.includes('panduan: {'));
  
  if (panduanIndex !== -1) {
    lines.splice(panduanIndex + 1, 0, data[lang].routeMapExt);
  }
});

fs.writeFileSync('src/translations/pagesTranslations.js', lines.join('\n'), 'utf8');
console.log('Successfully injected translated locations into routeMap (fixed version)!');
