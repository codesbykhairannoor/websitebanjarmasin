export const id = {
  common: {
    loading: "Memuat...",
    learnMore: "Pelajari Lebih Lanjut ➔",
    exploreNow: "Jelajahi Sekarang ➔",
    close: "Tutup",
    backToHome: "Kembali ke Beranda"
  },
  navbar: {
    brand: "Banjarmasin",
    aboutCity: "Tentang Kota",
    profile: "Profil Kota",
    history: "Sejarah 1526",
    innovation: "Inovasi Smart City",
    tourism: "Wisata",
    culinary: "Kuliner",
    culture: "Budaya & Seni",
    guide: "Panduan & Rute",
    exploreCTA: "Jelajah Kota",
    exploreCTA_mobile: "Jelajah Kota Sekarang ➔"
  },
  footer: {
    description: "Portal Resmi Eksplorasi Pariwisata & Budaya Bumi Lambung Mangkurat. Menyajikan panduan perjalanan tanpa batas di Kota Seribu Sungai.",
    mascotTitle: "Ditemani Maskot Bekantan",
    mascotSub: "Siaga 24 Jam di Siring Martapura",
    sectionTravel: "Jelajah Rasa",
    sectionAbout: "Tentang Kota",
    sectionEmergency: "Layanan & Siaga",
    emergencies: {
      callCenter: "🚨 Call Center",
      police: "👮‍♂️ Polairud Siring",
      hospital: "🏥 RSUD Ulin"
    },
    address: "📍 Balai Kota: Jl. RE Martadinata No.1, Banjarmasin Tengah, Kalimantan Selatan.",
    copyright: "© 2026 Pemerintah Kota Banjarmasin & Komunitas Wisata. All rights reserved."
  },
  assistant: {
    welcome: "Halo sanak! Ulun Acil AI Banjar 🐵✨ Siap memandu pian menjelajahi kuliner legendaris, wisata susur sungai, rute BRT, atau sejarah 1526. Ada yang handak ditanyakan?",
    thinking: "Acil sedang berpikir...",
    rateLimit: "🛡️ SUPER SECURITY ALERT: Sistem mendeteksi aktivitas pengiriman pesan terlalu cepat. Demi keamanan server & pencegahan spam, silakan tunggu beberapa saat atau langsung klik tombol WhatsApp Admin Manusia di bawah.",
    apiKeyWarning: "⚠️ API Key Gemini belum terdeteksi di file .env. Harap masukkan VITE_GEMINI_API_KEY Anda agar ulun bisa menjawab secara pintar! Untuk sementara, pian bisa langsung klik tombol WA Admin Manusia di bawah ya sanak.",
    errorMessage: "Mohon maaf sanak, koneksi AI ulun sedang terkendala gangguan jaringan. Pian bisa langsung bertanya ke Admin Manusia melalui tombol WhatsApp di bawah ini ya! 🙏",
    waButton: "Hubungi WA Admin Manusia",
    placeholder: "Ketik pertanyaan ke Acil...",
    title: "Acil AI Banjar",
    badge: "Gemini",
    status: "Siaga 24 Jam",
    quickQuestions: [
      "Rute bus BRT ke Pasar Terapung?",
      "Rekomendasi kuliner Soto Banjar enak?",
      "Aturan berpakaian ke Masjid Sultan Suriansyah?"
    ]
  },
  home: {
    hero: {
      slides: [
        {
          tag: "✦ GERBANG UTAMA KALIMANTAN",
          title: "Selamat Datang di Banjarmasin!",
          subtitle: "Pusat Peradaban & Pesona Jantung Borneo",
          desc: "Bersiaplah memulai penjelajahan tak terlupakan di Kota Seribu Sungai. Resapi eksotika budaya terapung yang melegenda, kelezatan rempah kuliner autentik, hingga pesona tata kota modern yang asri dan baiman.",
          price: "Eksplorasi Dimulai",
          btnText: "Mulai Petualangan ➔"
        },
        {
          tag: "✦ DESTINASI IKONIK",
          title: "Eksotika Kehidupan di Atas Air",
          subtitle: "Magisnya Fajar di Pasar Terapung & Susur Sungai",
          desc: "Menyaksikan langsung denyut nadi perdagangan tradisional di atas perahu jukung saat fajar menyingsing, dan rasakan ketenangan batin ketika menyusuri aliran Sungai Martapura yang membelah keindahan kota.",
          price: "Kearifan Lokal",
          btnText: "Buka Peta Wisata ➔"
        },
        {
          tag: "✦ WARISAN KULINER",
          title: "Simfoni Rempah Autentik Borneo",
          subtitle: "Kehangatan Soto Banjar & Ketupat Kandangan",
          desc: "Sebuah mahakarya rasa dari paduan kaldu rempah kapulaga, kelembutan perkedel, dan aroma khas ikan haruan panggang yang resep aslinya dijaga ketat melintasi berbagai generasi.",
          price: "Cita Rasa Legendaris",
          btnText: "Cicipi Menu Banjar ➔"
        },
        {
          tag: "✦ BUDAYA & SEJARAH",
          title: "Keanggunan Tradisi Leluhur",
          subtitle: "Estetika Sasirangan & Jejak Kesultanan",
          desc: "Menelusuri keagungan sejarah Kesultanan Banjar sejak tahun 1526, mengagumi kerumitan pola wastra Sasirangan pewarna alami, hingga menyaksikan kemegahan arsitektur kayu ulin peninggalan masa lalu.",
          price: "Jejak Peradaban",
          btnText: "Jelajahi Sejarah ➔"
        },
        {
          tag: "✦ BANJARMASIN PINTAR",
          title: "Smart City Seribu Sungai",
          subtitle: "Integrasi Pelayanan Digital & Ekologi",
          desc: "Wajah baru Banjarmasin yang memadukan ekosistem super-app pelayanan publik dengan komitmen pelestarian lingkungan sungai secara real-time, mewujudkan tata kota yang modern, bersih, dan inklusif.",
          price: "Kota Masa Depan",
          btnText: "Ekosistem Smart City ➔"
        }
      ]
    },
    wisata: {
      tag: "✦ EKSPLORASI ALAM & LANDMARK",
      title: "Destinasi Wisata Ikonik",
      desc: "Menyusuri denyut kehidupan sungai Martapura hingga pesona eksotis rimba Borneo yang tak akan terlupakan.",
      btnText: "Jelajahi Beragam Destinasi Alam & Budaya ➔",
      items: [
        { title: "Pasar Terapung Lok Baintan", category: "Budaya Sungai", time: "⏰ 05.30 - 08.00 WITA" },
        { title: "Menara Pandang & Siring", category: "Landmark Kota", time: "⏰ Terbuka 24 Jam" },
        { title: "Monumen Patung Bekantan", category: "Ikon Wisata", time: "⏰ 08.00 - 17.00 WITA" },
        { title: "Masjid Sultan Suriansyah", category: "Heritage 1526", time: "⏰ Wisata Religi" }
      ]
    },
    kuliner: {
      tag: "✦ WARISAN CITA RASA",
      title: "Kuliner Legendaris Banjar",
      desc: "Setiap hidangan adalah mahakarya rasa. Temukan resep rahasia dan perpaduan rempah khas Nusantara yang dijaga ketat oleh masyarakat Banjar melintasi berbagai generasi.",
      btnText: "Jelajahi Lokasi Kuliner Ini ➔",
      foods: [
        {
          name: "Soto Banjar Rempah Kapulaga",
          tabTitle: "🥣 Soto Banjar Rempah",
          aroma: "Harum Rempah Kapulaga, Kayu Manis & Cengkeh",
          desc: "Kaldu ayam kampung keemasan yang dimasak perlahan dengan rempah pilihan. Disajikan bersama suwiran daging ayam hangat, perkedel kentang lembut, dan ketupat pulen khas Banjar."
        },
        {
          name: "Ketupat Kandangan Haruan Asap",
          tabTitle: "🥘 Ketupat Kandangan",
          aroma: "Gurih Santan Kental & Aroma Smokey Ikan Haruan",
          desc: "Ketupat pulen yang disiram kuah santan kaya bumbu kuning tradisional. Dipadukan dengan lauk ikan haruan (gabus) panggang asap yang memberikan cita rasa mendalam yang tiada duanya."
        },
        {
          name: "Amparan Tatak Pisang",
          tabTitle: "🍌 Amparan Tatak",
          aroma: "Manis Legit Pisang & Gurih Santan Kukus",
          desc: "Kue tradisional (wadai) berlapis dua dengan tekstur lembut lumer di mulut. Lapisan bawah kaya akan potongan pisang manis, sementara lapisan atasnya adalah puding santan gurih yang pecah di lidah."
        },
        {
          name: "Bingka Kentang Kembang",
          tabTitle: "🍯 Bingka Kentang",
          aroma: "Legit Lembut Aroma Santan Bakar & Kentang",
          desc: "Kue basah tradisional berbentuk kelopak bunga kembang bundar. Memiliki tekstur super lembut dan manis legit alami dari campuran kentang premium serta santan kelapa bakar."
        },
        {
          name: "Sambal Acan Raja Banjar",
          tabTitle: "🌶️ Sambal Acan",
          aroma: "Pedas Menyengat & Terasi Fermentasi Harum",
          desc: "Oleh-oleh wajib khas Banjar! Sambal terasi (acan) yang difermentasi khusus dan dicampur dengan perasan mangga kuini atau ramania, menghasilkan sensasi pedas, asam, dan gurih yang luar biasa."
        }
      ]
    },
    budaya: {
      tag: "✦ WARISAN LELUHUR",
      title: "Jantung Budaya & Jejak Sejarah",
      desc: "Lebih dari sekadar kain dan transportasi, keduanya melambangkan ketangguhan serta harmoni manusiawi masyarakat Banjar dengan alam sungainya.",
      btnText: "Pelajari Kedalaman Budaya ➔",
      cards: [
        {
          title: "Kain Sasirangan",
          desc: "Dibuat melalui teknik menyirang dengan pewarna alami, dipercaya masyarakat sejak abad ke-16 sebagai wastra penyembuh magis."
        },
        {
          title: "Budaya Jukung",
          desc: "Perahu tradisional berbahan kayu ulin tanpa paku logam, didesain sempurna untuk membelah kuatnya arus sungai Kalimantan."
        }
      ],
      quote: "Di atas jukung, urat nadi kehidupan dan persaudaraan masyarakat Banjar terus mengalir melintasi lintas generasi dan zaman.",
      quoteAuthor: "— Pedagang Pasar Terapung"
    },
    smartCity: {
      tag: "✦ BANJARMASIN BAIMAN",
      title: "Kota Pintar Harmoni Ekologi",
      desc: "Integrasi cerdas antara super-app pelayanan digital masyarakat dengan komitmen memelihara kelestarian ekosistem sungai. Sebuah visi nyata menuju masa depan yang inklusif.",
      btnText: "Eksplorasi Ekosistem ➔",
      cards: [
        {
          title: "Super-App Banjarmasin Pintar",
          desc: "SSO NIK untuk akses semua layanan publik"
        },
        {
          title: "Pemantauan ATCS Terpusat",
          desc: "Kendali lalu lintas dan pantauan kota real-time"
        },
        {
          title: "E-Layanan Warga & UMKM",
          desc: "Integrasi kependudukan hingga e-Katalog UMKM"
        }
      ]
    },
    panduan: {
      tag: "✦ PERSIAPAN PERJALANAN",
      title: "Akomodasi & Transportasi",
      desc: "Panduan lengkap menuju pusat kota. Temukan hotel berkualitas serta moda transportasi umum andalan.",
      btnText: "Buka Survival Guide 2026 Lengkap ➔",
      cards: [
        {
          title: "Swiss-Belhotel Borneo",
          desc: "Berada tepat di tepi sungai dengan akses langsung ke dermaga kelotok privat."
        },
        {
          title: "Galaxy Hotel",
          desc: "Berada di gerbang masuk kota, cocok untuk mobilitas ekstra cepat via bandara."
        },
        {
          title: "BRT Trans Banjarbakula",
          desc: "Bus AC nyaman beroperasi menembus bandara hingga titik Nol Kilometer kota."
        }
      ]
    },
    planner: {
      tag: "✦ NAVIGASI PERJALANAN",
      title: "Rute & Perencana Perjalanan Wisata",
      desc: "Petakan petualangan Anda di Banjarmasin dengan kalkulator estimasi waktu tempuh antar spot-spot wisata legendaris secara interaktif.",
      selectTitle: "Pilih Destinasi Wisata:",
      routeTitle: "Rute Terbaik & Info Akses",
      timeTitle: "Estimasi Waktu Tempuh",
      transportTitle: "Transportasi Utama",
      bestTimeTitle: "Waktu Kunjungan Terbaik",
      highlightTitle: "Sorotan Utama Spot",
      ctaMaps: "Buka Google Maps ➔",
      spots: {
        lokbaintan: {
          title: "Pasar Terapung Lok Baintan",
          type: "Wisata Budaya Sungai",
          transport: "🛶 Perahu Kelotok (~35 Menit dari Dermaga Siring)",
          bestTime: "🌅 05.30 - 07.30 WITA (Matahari Terbit)",
          highlight: "Saksikan transaksi jual beli tradisional di atas jukung dengan sistem barter yang telah berlangsung sejak abad ke-16."
        },
        siring: {
          title: "Menara Pandang & Siring Sungai Martapura",
          type: "Landmark Pusat Kota",
          transport: "🚗 Jalan Kaki / Akses Kendaraan Umum",
          bestTime: "🌆 16.30 - 21.00 WITA (Senja & Malam Hari)",
          highlight: "Pusat rekreasi warga di tepian sungai Martapura. Nikmati hembusan angin sungai, live music, dan jajanan malam khas Banjar."
        },
        sotoamat: {
          title: "Dermaga Kuliner Soto Bang Amat",
          type: "Wisata Gastronomi",
          transport: "🛶 Kelotok Wisata / Mobil (~15 Menit dari Pusat Kota)",
          bestTime: "🍜 11.00 - 15.00 WITA (Makan Siang Live Panting)",
          highlight: "Menikmati semangkuk soto Banjar hangat tepat di pinggir sungai Martapura sembari ditemani alunan musik tradisional Panting."
        },
        pulaukembang: {
          title: "Delta Konservasi Pulau Kembang",
          type: "Wisata Alam & Satwa",
          transport: "Speedboat / Kelotok (~20 Menit Menyusuri Sungai Barito)",
          bestTime: "☀️ 08.00 - 11.00 WITA (Pagi Hari)",
          highlight: "Pulau delta di tengah sungai Barito yang menjadi habitat alami ribuan kera ekor panjang dan bekantan khas Borneo."
        }
      }
    }
  }
};
