    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center md:justify-end p-2 md:p-8 pointer-events-none animate-fade-in font-sans">
      <div className="max-w-[95vw] md:max-w-xl w-full glass-panel p-4 md:p-8 rounded-2xl md:rounded-3xl border border-white/20 shadow-2xl space-y-4 md:space-y-6 pointer-events-auto backdrop-blur-2xl relative overflow-hidden max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-1.5 md:space-y-2">
          ...
          <h2 className="text-xl sm:text-2xl md:text-3xl font-title font-black tracking-tight text-white capitalize leading-tight">
            {motif.title}
          </h2>
          <h3 className="text-xs sm:text-sm md:text-base font-game font-bold text-slate-300 italic tracking-wide">
            "{motif.subtitle}"
          </h3>
        </div>
        ...
        <div className="space-y-1.5 md:space-y-2">
          ...
          <p className="text-slate-200 text-xs sm:text-sm md:text-base leading-relaxed md:leading-loose text-justify p-3 md:p-4 rounded-xl md:rounded-2xl bg-slate-900/80 border border-white/15 font-medium md:font-semibold">
            {motif.philosophy}
          </p>
        </div>
        ...
        <div className="space-y-1.5 md:space-y-2">
          ...
          <div className="space-y-1.5 md:space-y-2">
            {motif.facts.map((fact, idx) => (
              <div key={idx} className="flex items-start gap-2.5 text-xs sm:text-sm md:text-base text-slate-300 p-2.5 md:p-3 rounded-xl bg-slate-900/60 font-medium md:font-semibold border border-white/5">
                <CheckCircle className="w-3.5 h-3.5 md:w-4 md:h-4 text-rose-400 shrink-0 mt-0.5" />
                <span>{fact}</span>
              </div>
            ))}
          </div>
