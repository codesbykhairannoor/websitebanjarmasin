const fs = require('fs');
const path = 'src/translations/pagesTranslations.js';
let content = fs.readFileSync(path, 'utf8');

const idInteractiveMap = `
      interactiveMap: {
...
let newContent = content.replace(/(wisata: \{\s*)(heroTag: "✦ EKSPLORASI DESTINASI BORNEO",)/, '$1' + idInteractiveMap + '\n      $2');
newContent = newContent.replace(/(wisata: \{\s*)(heroTag: "✦ BORNEO DESTINATION EXPLORATION",)/, '$1' + enInteractiveMap + '\n      $2');

// Handle MS which shares the same heroTag as ID
const parts = newContent.split('wisata: {');
if(parts.length > 3) {
  parts[3] = parts[3].replace(/(heroTag: "✦ EKSPLORASI DESTINASI BORNEO",)/, msInteractiveMap + '\n      $1');
  newContent = parts.join('wisata: {');
}

newContent = newContent.replace(/(wisata: \{\s*)(heroTag: "✦ 婆罗洲目的地探索",)/, '$1' + zhInteractiveMap + '\n      $2');

fs.writeFileSync(path, newContent, 'utf8');
console.log('Translations updated in pagesTranslations.js');
